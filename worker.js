...
export default {async fetch(request,env){
  ...
  if(url.pathname==="/api/generate"&&request.method==="POST"){
    try{
      ...
      const emotionMap={
        "فخر":"majestic pride, achievement, dignity, national celebration",
        "انتماء":"belonging, roots, family, homeland, cultural identity",
        "أمل":"hope, sunrise, renewal, bright horizon, optimism",
        "طموح":"ambition, progress, innovation, modern development",
        "امتنان":"gratitude, warmth, cherished heritage, peaceful joy",
        "مستقبل":"future, technology, sustainable development, modern Saudi city",
        "اعتزاز":"honor, heritage pride, identity, dignity, cultural confidence",
        "فرح":"joy, festive atmosphere, celebration, fireworks, joyful light"
      };
      ...
      Object.keys(keywordMap).forEach(function(k){
        if(feelings.includes(k)&&detected.indexOf(keywordMap[k])===-1)detected.push(keywordMap[k]);
      });

      const chosen=(chips+"، "+detected.join("، "))
        .split("،").map(function(x){return x.trim()}).filter(Boolean);

      const moods=chosen.map(function(x){return emotionMap[x]||""})
        .filter(Boolean).filter(function(x,i,a){return a.indexOf(x)===i})
        .join("; ");

      const visualMood=moods||"national pride, belonging, hope, joyful celebration";

      const prompt=
        "Create one original, highly realistic premium Saudi National Day 96 exhibition artwork. "+
        "This must look like a believable professional Saudi cultural campaign photograph, not fantasy and not generic desert art. "+
        "Emotional direction: "+visualMood+". "+
        "Show a coherent Saudi scene combining REALISTIC Najdi heritage and REALISTIC modern Saudi development: authentic Diriyah-style mud-brick heritage architecture and palm oasis in the foreground, subtle traditional Saudi geometric and Sadu-inspired decorative details, with a recognizable modern Riyadh skyline and contemporary Saudi architecture in the distance. "+
        "Create a genuine National Day celebration atmosphere using elegant emerald-green and white architectural lighting, tasteful festive light installations and realistic fireworks in the evening sky. "+
        "The composition should clearly communicate Saudi identity, pride in the homeland, heritage, development and optimism for the future. "+
        "Use natural materials, realistic architecture, physically plausible proportions, realistic lighting, photographic depth, premium editorial photography, cinematic but believable, refined composition, rich green, white, warm gold and natural earth tones. "+
        "No fantasy buildings, no impossible structures, no distorted architecture, no giant objects, no surreal landscape, no random airplanes, no crowds, no close-up faces. "+
        "No flags, no banners, no signs, no screens, no plaques, no logos and no objects containing writing. "+
        "ABSOLUTELY NO TEXT OR LETTERS anywhere: no Arabic, no English, no numbers, no calligraphy, no captions, no watermark, no signature, no readable symbols. "+
        "No poster, no infographic, no illustration, no cartoon, no anime, no collage, no split panels. "+
        "Make the National Day celebration unmistakable through Saudi heritage, modern Saudi progress, green-and-white festive lighting and fireworks.";
      ...
      if(!result||!result.image)throw Error("لم تُرجع خدمة الذكاء الاصطناعي صورة");
      return Response.json({image:"data:image/jpeg;base64,"+result.image});
    }catch(e){
      return Response.json({error:e&&e.message?e.message:"حدث خطأ أثناء إنشاء اللوحة"},{status:500});
    }
  }
