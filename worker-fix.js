...
<section class="card"><h2>🖼️ معرض اللوحات المشترك · اليوم الوطني 96</h2><div id="gallery" class="gallery"><div class="empty">جاري تحميل المعرض...</div></div></section>
...
async function render(){
 try{var r=await fetch("/api/gallery",{cache:"no-store"});var a=await r.json();if(!r.ok)throw Error();var g=document.getElementById("gallery");g.innerHTML=a.length?a.map(function(x){
  var dt=x.createdAt?new Date(x.createdAt).toLocaleString("ar-SA",{dateStyle:"short",timeStyle:"short"}):"";
  return '<div class="gitem"><img src="'+x.image+'" alt="لوحة اليوم الوطني السعودي 96"><div class="gname"><b>'+esc(x.name||"مشاركة")+"</b><br><span>"+esc(dt)+"</span></div></div>"
}).join(""):'<div class="empty">لم تُحفظ لوحات بعد 🌿</div>'}catch(e){document.getElementById("gallery").innerHTML='<div class="empty">تعذر تحميل المعرض الآن.</div>'}}
render();
...
async function saveToGallery(auto){
 ...
 b.disabled=true;
 if(auto)msg("💚 جارٍ حفظ اللوحة تلقائيًا في المعرض المشترك...");
 try{
  var r=await fetch("/api/gallery",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
   image:currentImage,
   name:document.getElementById("name").value.trim()||"مشاركة"
  })});
  var d=await r.json().catch(function(){return {error:"تعذر قراءة نتيجة الحفظ"}});
  if(!r.ok)throw Error(d.error||"تعذر الحفظ في المعرض");
  b.textContent="✅ تم الحفظ في المعرض";
  await render();
  msg(auto?"💚 تم حفظ اللوحة تلقائيًا في معرض اللوحات المشترك.":"💚 تم حفظ اللوحة في معرض اللوحات المشترك.");
  return true;
 }catch(e){
  msg("⚠️ لم يتم الحفظ في المعرض المشترك: "+e.message);
  return false;
 }finally{
  b.disabled=false;
 }
